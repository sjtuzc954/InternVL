...
if __name__ == '__main__':
    ...
    with open(meta_path, encoding="UTF-8") as f:
        ...
        annotation = metadata['ui-dataset']['annotation']
    ...
    with open(annotation, encoding="UTF-8") as f:
        for jsonl in jsonlines.Reader(f):
            ...
            if i1 != -1 and i2 != -1:
                ...
                coords = [float(i) for i in coord_substr.split(', ')]
                img = Image.open(os.path.join(root, image_path))
                draw = ImageDraw.ImageDraw(img)
                draw.rectangle(((coords[0] / 1000 * img.width, coords[1] / 1000 * img.height), (coords[2] / 1000 * img.width, coords[3] / 1000 * img.height)), outline='red', width=2)
                img.save(f'eval/ui/results/{image_path}')
